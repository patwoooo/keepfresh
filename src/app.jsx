...
function PickerColumn({ items, selected, onSelect }) {
  ...
  return (
    <div style={{ flex: 1, position: "relative", height: ITEM_H * VISIBLE, overflow: "hidden" }}>
      ...
      <div ref={ref} onScroll={handleScroll} style={{ height: "100%", overflowY: "scroll", scrollSnapType: "y mandatory", scrollbarWidth: "none", paddingTop: ITEM_H * 3, paddingBottom: ITEM_H * 3 }}>
        {items.map(item => (
          <div key={item} onClick={() => { onSelect(item); ref.current.scrollTo({ top: items.indexOf(item) * ITEM_H, behavior: "smooth" }); }}
            style={{ height: ITEM_H, display: "flex", alignItems: "center", justifyContent: "center", scrollSnapAlign: "start", fontFamily: NOTO, cursor: "pointer", fontSize: item === selected ? 20 : 16, fontWeight: item === selected ? 700 : 400, color: item === selected ? "#111" : "#aaa", transition: "all 0.15s" }}>
            {item}
          </div>
        ))}
      </div>
...
function AddScreen({ image, onClose, onSave, categories }) {
  ...
  useEffect(() => { const max = daysInMonth(year, month); if (day > max) setDay(max); }, [year, month]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const [saved] = await dbFetch("items?select=*", {
        method: "POST",
        body: JSON.stringify({ name: name || "食材", category, expiry: expiryStr, image_url, qty, memo }),
      });
      onSave(saved);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#f5f5f5", zIndex: 400, maxWidth: 375, margin: "0 auto", overflowY: "auto", fontFamily: NOTO }}>
      ...
      <div style={{ padding: "60px 28px 20px", display: "flex", justifyContent: "center" }}>
        <div style={{ width: "100%", maxWidth: 320, aspectRatio: "1/1", overflow: "hidden", position: "relative", background: image ? "transparent" : "#5b8fa8", boxShadow: "0 4px 24px rgba(0,0,0,0.13)" }}>
          {image ? <img src={image} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.5)", fontSize: 13 }}>無照片</div>}
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent, rgba(0,0,0,0.62))", padding: "32px 0 10px", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 1 }}>
              {daysLeft < 0 ? <span style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 14, color: "#fff" }}>已過期</span>
                : <><span style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 20, color: "#fff", lineHeight: 1 }}>{daysLeft}</span><span style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 9, color: "#fff", marginLeft: 1 }}>日</span></>}
            </div>
            <div style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 9, color: "rgba(255,255,255,0.85)", textAlign: "center", marginTop: 2 }}>{formatDate(expiryStr)}</div>
          </div>
        </div>
      </div>
      <div style={{ background: "#fff", margin: "0 0 8px", padding: "0 20px" }}>
        <div style={{ borderBottom: "1px solid #eee", padding: "14px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="食材名稱" style={{ border: "none", outline: "none", fontSize: 15, fontFamily: NOTO, textAlign: "right", color: "#111", background: "transparent", width: "55%" }} />
        </div>
        <div style={{ borderBottom: "1px solid #eee", padding: "12px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <button onClick={() => setQty(q => Math.max(1, q - 1))} style={{ width: 32, height: 32, borderRadius: "50%", border: "1.5px solid #34a853", background: "#fff", fontSize: 18, color: "#34a853", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
            <span style={{ fontSize: 17, fontWeight: 500, minWidth: 20, textAlign: "center" }}>{qty}</span>
            <button onClick={() => setQty(q => q + 1)} style={{ width: 32, height: 32, borderRadius: "50%", border: "1.5px solid #34a853", background: "#fff", fontSize: 18, color: "#34a853", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #eee", paddingBottom: 4, background: "#f9f9f9", borderRadius: 10, margin: "6px 0 0" }}>
        ...
        <div style={{ borderBottom: "1px solid #eee", padding: "14px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <select value={category} onChange={e => setCategory(e.target.value)} style={{ border: "none", outline: "none", fontSize: 15, fontFamily: NOTO, color: "#111", background: "transparent", textAlign: "right" }}>
        ...
        <div style={{ padding: "14px 0" }}>
          ...
          <textarea value={memo} onChange={e => setMemo(e.target.value)} placeholder="備註（選填）" rows={2} style={{ width: "100%", border: "none", outline: "none", fontSize: 14, fontFamily: NOTO, color: "#111", background: "transparent", resize: "none", boxSizing: "border-box" }} />
        </div>
      </div>
      <div style={{ padding: "16px 20px 40px" }}>
        <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: "15px", background: saving ? "#aaa" : "#34a853", border: "none", borderRadius: 30, color: "#fff", fontSize: 17, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", fontFamily: NOTO }}>
          {saving ? "儲存中..." : "新增"}
        </button>
      </div>
    </div>
...
export default function KeepFresh() {
  ...
  useEffect(() => { const t = setInterval(loadItems, 30000); return () => clearInterval(t); }, [loadItems]);
  ...
  function handleSaveItem(saved) { setItems(prev => [saved, ...prev]); setAddScreen(null); }
    ...
    <>
      ...
      <div style={{ fontFamily: NOTO, background: "#f2f4f3", minHeight: "100vh", maxWidth: 375, margin: "0 auto", position: "relative", paddingBottom: 110 }}>
        <div style={{ background: "#fff", padding: "14px 16px 0", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center", padding: "6px 0 10px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#f0f0f0", borderRadius: 8, padding: "7px 10px", flex: 1 }}>
              <IconSearch />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜尋食材..." style={{ border: "none", background: "transparent", fontSize: 13, outline: "none", width: "100%", fontFamily: NOTO }} />
            </div>
            <div style={{ position: "relative" }}>
              <button onClick={() => setShowSortDropdown(p => !p)} style={{ background: "#f0f0f0", border: "none", borderRadius: 8, padding: "7px 10px", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 3, fontFamily: NOTO, color: "#444", whiteSpace: "nowrap" }}>
                {sortBy} <IconChevron />
              </button>
              {showSortDropdown && (
                <div style={{ position: "absolute", right: 0, top: 36, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.12)", zIndex: 200, minWidth: 150, overflow: "hidden" }}>
                  {SORT_OPTIONS.map(opt => (
                    <div key={opt} onClick={() => { setSortBy(opt); setShowSortDropdown(false); }} style={{ padding: "10px 14px", fontSize: 13, cursor: "pointer", fontFamily: NOTO, background: sortBy === opt ? "#f0faf3" : "#fff", color: sortBy === opt ? "#34a853" : "#333", fontWeight: sortBy === opt ? 700 : 400 }}>
                      {opt}
                    </div>
                  ))}
                </div>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 0 }}>
            {filtered.length === 0 ? (
              ...
              return (
                <div key={item.id} onClick={() => setSelectedItem(item)} style={{ position: "relative", aspectRatio: "1 / 1.15", background: item.image_url ? "#eee" : BG_COLORS[idx % BG_COLORS.length], cursor: "pointer", overflow: "hidden" }}>
                  {item.image_url
                  ...
                  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent, rgba(0,0,0,0.62))", padding: "24px 0 7px", display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 1 }}>
                      {days < 0 ? <span style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 14, color: "#fff" }}>已過期</span>
                        : <><span style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 20, color: "#fff", lineHeight: 1 }}>{days}</span><span style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 9, color: "#fff", marginLeft: 1 }}>日</span></>}
                    </div>
                    <div style={{ fontFamily: NOTO, fontWeight: 300, fontSize: 9, color: "rgba(255,255,255,0.85)", textAlign: "center", marginTop: 2 }}>{formatDate(item.expiry)}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 375, display: "flex", justifyContent: "center", padding: "0 0 32px", zIndex: 100, pointerEvents: "none" }}>
          <button onClick={handleFabClick} style={{ pointerEvents: "all", background: "#34a853", border: "none", borderRadius: "50%", width: 90, height: 90, fontSize: 36, color: "#fff", cursor: "pointer", boxShadow: "0 6px 24px rgba(52,168,83,0.45)", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
        </div>

        {addScreen && <AddScreen image={addScreen.image} categories={categories} onClose={() => setAddScreen(null)} onSave={handleSaveItem} />}

        {showAddCategory && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 300, display: "flex", alignItems: "flex-end" }} onClick={e => e.target === e.currentTarget && setShowAddCategory(false)}>
            <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 375, margin: "0 auto", padding: "24px 20px 44px" }}>
              ...
              <input value={newCatName} onChange={e => setNewCatName(e.target.value)} placeholder="輸入分類名稱" style={{ width: "100%", padding: "10px 12px", border: "1px solid #e0e0e0", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: NOTO, borderRadius: 8, marginBottom: 16 }} />
              <button onClick={handleAddCategory} style={{ width: "100%", padding: "13px", background: newCatName ? "#34a853" : "#e0e0e0", border: "none", borderRadius: 10, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: NOTO }}>新增分類</button>
            </div>
          </div>
        )}

        {selectedItem && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 300, display: "flex", alignItems: "flex-end" }} onClick={e => e.target === e.currentTarget && setSelectedItem(null)}>
            <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 375, margin: "0 auto", padding: "24px 20px 44px" }}>
              {selectedItem.image_url && <img src={selectedItem.image_url} style={{ width: "100%", aspectRatio: "1/1", objectFit: "cover", marginBottom: 14 }} />}
              <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 5, fontFamily: NOTO }}>{selectedItem.name}</div>
              <div style={{ color: "#888", fontSize: 13, marginBottom: 3, fontFamily: NOTO }}>分類：{categories.find(c => c.id === selectedItem.category)?.label}</div>
              <div style={{ color: "#888", fontSize: 13, marginBottom: 3, fontFamily: NOTO }}>數量：{selectedItem.qty}</div>
              <div style={{ color: "#888", fontSize: 13, marginBottom: selectedItem.memo ? 3 : 14, fontFamily: NOTO }}>到期日：{formatDate(selectedItem.expiry)}</div>
              {selectedItem.memo && <div style={{ color: "#888", fontSize: 13, marginBottom: 14, fontFamily: NOTO }}>備註：{selectedItem.memo}</div>}
              <div style={{ fontSize: 26, fontWeight: 300, marginBottom: 18, fontFamily: NOTO, color: "#34a853" }}>
                {daysUntil(selectedItem.expiry) < 0 ? "已過期！" : `還有 ${daysUntil(selectedItem.expiry)} 天`}
              </div>
              <button onClick={() => handleDeleteItem(selectedItem.id)} style={{ width: "100%", padding: "13px", background: "#d32f2f", border: "none", borderRadius: 10, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: NOTO }}>刪除此食材</button>
            </div>
          </div>
        )}
      </div>
